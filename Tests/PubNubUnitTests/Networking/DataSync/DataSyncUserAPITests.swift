...
final class DataSyncUserAPITests: DataSyncAPITestCase {
  func test_GetUsers_DecodesItemsAndNextPage() throws {
    ...
    pubnub.dataSync.getUsers(limit: 20) { [self] result in
      ...
      case let .success((users, next)):
        ...
        XCTAssertEqual(users[0].className, "User")
  ...
  func test_CreateUser_WithClassNameAndLevel_SendsThemInBody() throws {
    let expectation = self.expectation(description: "createUser class identity")
    let sessions = try MockURLSession.mockSession(for: ["datasync_user_fetch_success"])
    let pubnub = TestPubNubFactory.make(session: sessions.session)
    ...
    XCTAssertEqual(body["entityClass"]?.stringOptional, "Admin")
    XCTAssertEqual(body["entityClassVersion"]?.intOptional, 2)
    XCTAssertEqual(body["entityClassLevel"]?.stringOptional, "SubKey")
  }

  func test_CreateUser_WithoutClassNameAndLevel_OmitsThemFromBody() throws {
    let expectation = self.expectation(description: "createUser default class identity")
    let sessions = try MockURLSession.mockSession(for: ["datasync_user_fetch_success"])
    let pubnub = TestPubNubFactory.make(session: sessions.session)

    pubnub.dataSync.createUser(classVersion: 1, id: "alice") { _ in
      expectation.fulfill()
    }
    ...
    XCTAssertNil(body["entityClass"])
    XCTAssertNil(body["entityClassLevel"])
  }
