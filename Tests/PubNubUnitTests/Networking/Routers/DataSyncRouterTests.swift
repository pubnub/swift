...
extension DataSyncRouterTests {
  ...
  func test_EntityCreate_SendsEntityClassLevel() throws {
    let router = DataSyncEntityRouter(
      .create(body: .init(entityClass: "Patient", entityClassVersion: 1, entityClassLevel: "SubKey")),
      configuration: config
    )
  ...
  func test_EntityCreate_OmitsEntityClassLevelWhenNil() throws {
    let router = DataSyncEntityRouter(
      .create(body: .init(entityClass: "Patient", entityClassVersion: 1)),
      configuration: config
    )

    XCTAssertNil(try decodeBody(router)["entityClassLevel"])
  }
