    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 3) { [unowned client] in
      client.fetchMessageHistory(
        ...
        case let .success(response):
          XCTAssertEqual(response.messagesByChannel.count, 1)
          XCTAssertEqual(
            response.messagesByChannel[channel]?.count,
            (startIndex.advanced(by: 1)...endIndex).count
          )
          XCTAssertEqual(
            response.messagesByChannel[channel]?.compactMap { $0.payload.stringOptional },
            Array(messagesToSend[startIndex.advanced(by: 1)...endIndex])
          )
        case let .failure(error):
          XCTFail("Failed to fetch history: \(error)")
        }
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 3) { [unowned client] in
      client.fetchMessageHistory(
        ...
        case let .success(response):
          ...
          XCTAssertEqual(
            response.messagesByChannel[channel]?.compactMap { $0.metadata?.stringOptional },
            messagesToSend.map { $0 + "meta" }
          )
          XCTAssertEqual(
            response.messagesByChannel[channel]?.compactMap { $0.customMessageType?.stringOptional },
            messagesToSend.map { $0 + "type" }
          )
          XCTAssertEqual(
            response.messagesByChannel[channel]?.compactMap { $0.publisher?.stringOptional },
            messagesToSend.map { _ in client.configuration.userId }
          )
          XCTAssertEqual(
            response.messagesByChannel[channel]?.compactMap { $0.messageType.rawValue },
            messagesToSend.map { _ in PubNubMessageType.message.rawValue }
          )
        case let .failure(error):
          XCTFail("Failed to fetch history: \(error)")
        }
